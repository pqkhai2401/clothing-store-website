...
@section('content')
    <main class="app-main container-fluid py-4">
        ...
        <div class="d-flex align-items-start justify-content-between gap-3 mb-4 flex-wrap">
            <h1 class="h4 fw-bold mb-0" style="color:#174761;">Quản lý kích thước</h1>
            <div class="small text-muted">Trang chủ <span class="mx-1">/</span> Kích thước</div>
        </div>
        ...
        <div class="d-flex align-items-center gap-2 mb-3">
            ...
            <a href="{{ route('admin.sizes.trash') }}" class="btn btn-outline-secondary page-action-btn">
        ...
        <div class="card border shadow-sm">
            <div class="card-header bg-white border-bottom">
                <form method="GET" action="{{ route('admin.sizes.list') }}">
                    <div class="row g-2 align-items-center">
                        <div class="col-md-5 col-lg-4">
                            <div class="input-group">
                                <span class="input-group-text bg-white"><i class="fa-solid fa-magnifying-glass"></i></span>
                                <input type="text" name="keyword" class="form-control"
                                    value="{{ $keyword }}" placeholder="Tìm theo tên kích thước...">
                            </div>
                        </div>
                        <div class="col-auto">
                            ...
                            @if($keyword)
                                <a href="{{ route('admin.sizes.list') }}" class="btn btn-outline-secondary fw-semibold ms-1">
                    ...
                    <table class="table table-hover table-bordered mgmt-table align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($sizes as $size)
                                <tr>
                                    <td class="hk-cb-td"><input type="checkbox" class="hk-cb-row" value="{{ $size->id }}"></td>
                                    <td class="ps-3" style="opacity:.45;">{{ $size->id }}</td>
                                    <td class="fw-semibold">{{ $size->name }}</td>
                                    <td><span class="count-num">{{ $size->product_variants_count }}</span></td>
                                    <td class="text-muted" style="font-size:12px;">
                                        {{ $size->created_at?->format('d/m/Y') ?? '—' }}
                                    </td>
                                    <td class="text-center">
                                        <div class="d-inline-flex gap-1">
                                            <a href="{{ route('admin.sizes.edit', $size->id) }}" class="btn btn-sm btn-outline-warning" title="Sửa">
            ...
            <div class="card-footer bg-white">
                @include('layouts.components.pagination', ['paginator' => $sizes, 'itemLabel' => 'kích thước', 'bulkDeleteUrl' => route('admin.sizes.bulkDelete')])
            </div>
        </div>
    </main>
@endsection
