...
@section('title', 'Sửa Bộ sưu tập: ' . $collection->name)
...
<main class="app-main container-fluid py-4">
    ...
    <div class="d-flex align-items-center justify-content-between gap-3 mb-4 flex-wrap">
        <div>
            <h1 class="h4 fw-bold mb-1" style="color:#174761;">Sửa Bộ sưu tập</h1>
            <p class="mb-0 text-muted" style="font-size:13px;">ID: {{ $collection->id }} — <strong>{{ $collection->name }}</strong></p>
        </div>
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <input type="text" id="name" name="name" 
                                   ...
                                   value="{{ old('name', $collection->name) }}" placeholder="Ví dụ: Bộ sưu tập Xuân 2026" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="slug" name="slug" 
                                   ...
                                   value="{{ old('slug', $collection->slug) }}" placeholder="Tự động tạo từ tên nếu bỏ trống">
                            @error('slug')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="description" name="description" rows="4" 
                                      ...
                                      placeholder="Mô tả phong cách, chất liệu, ý nghĩa của BST này...">{{ old('description', $collection->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            @if($collection->banner)
                                <div class="mb-2">
                                    <img src="{{ asset($collection->banner) }}" alt="Current Banner" 
                                         class="img-thumbnail d-block mb-1" style="max-height: 120px; object-fit: cover;">
                                    <span class="text-muted" style="font-size: 11px;">Banner hiện tại</span>
                                </div>
                            @endif
            ...
            <div class="col-lg-6 mb-4">
                <div class="card shadow-sm h-100 d-flex flex-column">
                    <div class="card-header bg-white py-3 d-flex align-items-center justify-content-between">
                        <span class="fw-bold text-uppercase text-secondary" style="font-size: 13px;">Sản phẩm trong BST</span>
                        <span class="badge bg-dark rounded-pill" id="selectedCount">Đã chọn: {{ count($selectedProductIds) }}</span>
                    </div>
                    <div class="card-body p-0 flex-grow-1" style="max-height: 480px; overflow-y: auto;">
                        ...
                        <div class="list-group list-group-flush" id="productList">
                            @forelse($products as $product)
                                ...
                                @endphp
                                <label class="list-group-item d-flex align-items-center gap-3 py-2 px-3 product-item {{ $isChecked ? 'bg-light-blue' : '' }}" style="cursor: pointer;">
                                    <input class="form-check-input flex-shrink-0 product-checkbox" type="checkbox" name="products[]" 
...
<style>
    .bg-light-blue {
        background-color: rgba(23, 71, 97, 0.04) !important;
    }
</style>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Cập nhật số lượng đã chọn và background highlight
