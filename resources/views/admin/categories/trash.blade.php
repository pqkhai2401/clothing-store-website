    ...
    <main class="app-main container-fluid py-4">
        ...
        <div class="d-flex align-items-start justify-content-between gap-3 mb-4 flex-wrap">
            <h1 class="h4 fw-bold mb-0" style="color:#174761;">
                <i class="fa-solid fa-trash me-2 text-danger" style="font-size:18px;"></i>Thùng rác — Danh mục
            </h1>
            <div class="small text-muted">Trang chủ <span class="mx-1">/</span> <a href="{{ route('admin.categories.list') }}" class="text-muted">Danh mục</a> <span class="mx-1">/</span> Thùng rác</div>
        </div>

        <div class="mb-3">
            <a href="{{ route('admin.categories.list') }}" class="btn btn-outline-secondary page-action-btn">
                ...
                <form method="GET" action="{{ route('admin.categories.trash') }}">
                    <div class="row g-2 align-items-center">
                        ...
                        <div class="col-auto">
                            ...
                            @if($keyword)
                                <a href="{{ route('admin.categories.trash') }}" class="btn btn-outline-secondary fw-semibold ms-1">
                    ...
                    <table class="table table-hover table-bordered mgmt-table align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($categories as $category)
                                <tr>
                                    <td class="ps-3" style="opacity:.45;">{{ $category->id }}</td>
                                    <td class="fw-semibold text-muted">{{ $category->name }}</td>
                                    <td class="text-muted" style="font-size:12px;">{{ $category->parentCategory?->name ?? '—' }}</td>
                                    <td><span class="deleted-at">{{ $category->deleted_at?->format('d/m/Y H:i') }}</span></td>
