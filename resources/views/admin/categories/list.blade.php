...
@section('content')
    <main class="app-main container-fluid py-4">
        ...
        <div class="d-flex align-items-start justify-content-between gap-3 mb-4 flex-wrap">
            <h1 class="h4 fw-bold mb-0" style="color:#174761;">Quản lý danh mục</h1>
            <div class="small text-muted">Trang chủ <span class="mx-1">/</span> Danh mục</div>
        </div>
        ...
        <div class="card border shadow-sm">
                        ...
                        <div class="col-md-5 col-lg-4">
                            <div class="input-group">
                                ...
                                <input type="text" name="keyword" class="form-control"
                                    value="{{ $keyword }}" placeholder="Tìm theo tên danh mục...">
                            </div>
                        </div>
                        <div class="col-auto">
                            ...
                            @if($keyword)
                                <a href="{{ route('admin.categories.list') }}" class="btn btn-outline-secondary fw-semibold ms-1">
                    ...
                    <table class="table table-hover table-bordered mgmt-table align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($categories as $category)
                                <tr>
                                    <td class="hk-cb-td"><input type="checkbox" class="hk-cb-row" value="{{ $category->id }}"></td>
                                    <td class="ps-3" style="opacity:.45;">{{ $category->id }}</td>
                                    <td>
                                        <span class="fw-semibold">{{ $category->name }}</span>
                                        @if(is_null($category->parent_id))
                                            <span class="ms-2 parent-badge">Cha</span>
                                        @else
                                            <span class="ms-2 child-badge">Con</span>
                                        @endif
                                    </td>
            ...
            <div class="card-footer bg-white">
                @include('layouts.components.pagination', ['paginator' => $categories, 'itemLabel' => 'danh mục', 'bulkDeleteUrl' => route('admin.categories.bulkDelete')])
            </div>
        </div>
    </main>
@endsection
